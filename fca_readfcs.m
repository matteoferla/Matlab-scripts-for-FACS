...
%
% 15/04/2017
% First try for the FCS 3.1 standard
...
elseif  strcmp(fcsheader_type,'FCS2.0') || strcmp(fcsheader_type,'FCS3.0')  || strcmp(fcsheader_type,'FCS3.1') % FCS2.0 or FCS3.0 or FCS3.1 types
    ...
    elseif fcsheader_main(1) == 30
        % "Record Selector" asci code = 30, added by L Balkay 2015
...
%% this is for Gy Vamosi converting MoFlo Astrios with changing some hdr parameters
%% 11/09/2015
save_MoFlo_Astrios = 1;
...
elseif strcmp(mnemonic_separator, 'RS') % Laszlo Balkay, 21/09/2015
    mnemonic_startpos = findstr(char(fcsheader'),mnemonic_name);
    if isempty(mnemonic_startpos)
        mneval = [];
        return;
